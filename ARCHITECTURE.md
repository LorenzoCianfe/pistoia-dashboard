...
| ~~**`<ViewTransition>` di React non è disponibile**~~ | ✅ **Superata il 2026-08-05 con Next 16.3.0**: il flag `experimental.viewTransition` è stato rimosso e l'integrazione è di default. `<ViewTransition>` copre il cross-fade (in `(app)/template.tsx`); il **morph lista→dettaglio** resta l'API nativa a mano — per la ragione architetturale di `DESIGN.md` §7 (l'originale è già smontato), non per indisponibilità |
| **In sviluppo la CSP non ha `'strict-dynamic'`** | Deciso il 2026-08-05 per sbloccare Next 16.3, che in sviluppo emette un `<script>` senza nonce con dentro codice dell'applicazione. In **produzione** `'strict-dynamic'` è intatto. Da rimettere anche in sviluppo quando Next rimetterà il nonce: la condizione verificabile è in `SECURITY.md` §7 |
...
| ~~Nessun test automatico di accessibilità~~ | ✅ **2026-08-05**: `tests/e2e/accessibilita.spec.ts` (axe-core, 8 pagine × 2 temi, WCAG AA, nessuna esclusione). I contrasti dell'ondata 6 erano stati misurati **a mano una volta sola** e **non reggevano**: il debito emerso è stato chiuso scurendo la tavolozza chiara (`DESIGN.md` §4). Resta scoperto ciò che axe non vede: ordine di lettura, trappole di focus, sensatezza degli annunci |
