...
| **Il footer non è un punto di riferimento** | `A11Y` | 🔴 aperto il 2026-08-05, e **preesistente**. `<Footer />` è reso DENTRO `<main>` — in `AppShell` come nel layout `(pubblico)` — e un `<footer>` discendente di `main` **non è mappato a `contentinfo`**: chi naviga a landmark non ha modo di saltarci, su nessuna pagina della piattaforma. Si chiude spostandolo fuori da `<main>` (una riga in due layout), ma **`AppShell` è nell'elenco dei file protetti**: serve un sì esplicito. La regola axe che lo direbbe, `landmark-contentinfo-is-top-level`, è taggata `best-practice` e resta fuori dal cancello |
...
| **§11.6 «target ≥ 44px» non è trasformabile in un cancello** | `A11Y` `DOC` | 🔴 **aperto il 2026-08-06, ed è un problema di documento prima che di codice.** WCAG 2.5.8 chiede 24px **con quattro eccezioni** (spaziatura, inline, equivalente, essenziale); `DESIGN.md` §11.6 chiede 44 **senza eccezioni**. Misurati con metro crudo, sulle 8 pagine del cancello ci sono **246 elementi interattivi sotto i 44px** — e quasi tutti sono legittimi: link dentro la prosa, che a 44px spaccherebbero il testo. Conseguenza: la regola non funziona da vincolo, funziona da aspirazione, e infatti i 16px del footer sono sopravvissuti per mesi. **Decisione richiesta**: dare a §11.6 le stesse eccezioni di WCAG (e allora diventa un cancello scrivibile), oppure limitarla ai bersagli **isolati e tattili** — pulsanti, icone, righe di lista — lasciando fuori la prosa |
