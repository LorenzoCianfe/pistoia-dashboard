...
| **Spezzare `/admin` in sotto-rotte** | `UX` `FE` | 📋 **piano scritto e deciso il 2026-08-07, esecuzione da fare** — [`docs/piano-admin.md`](docs/piano-admin.md). Sette pagine, e il taglio l'hanno deciso le misure: «Proposte cittadine» da sola fa **1.710px**, quindi dieci pagine non sono meglio di sette (stesso massimo, tre rotte in più, quattro pagine da un modulo solo), e il taglio «per verbo» in tre lasciava una pagina da **4.593px**. Ne è uscita una regola vincolante, ora in `DESIGN.md` §6: *una coda una pagina · gli strumenti insieme · le letture sul cruscotto · il registro è una lettura*. Entra **fuori ordine** rispetto a O8, per decisione di Lorenzo |
