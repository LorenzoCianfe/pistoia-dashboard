...
| **Un cancello per la dimensione dei bersagli** | `A11Y` `ENG` | 🔴 aperto il 2026-08-05. `accessibilita.spec.ts` gira sui tag `wcag2aa`/`wcag21aa`, e `target-size` è **WCAG 2.2**: il vincolo di `DESIGN.md` §11.6 non è misurato da nessuno, ed è per questo che i link del footer sono stati alti 16px su ogni pagina senza che nulla lo dicesse. Il minimo utile: aggiungere il tag `wcag22aa` al cancello e leggere quanto rosso esce prima di decidere se renderlo bloccante |
