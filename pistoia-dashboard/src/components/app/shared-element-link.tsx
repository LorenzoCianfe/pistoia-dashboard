...
import { NOME_CONDIVISO, type ElementoCondiviso } from "@/lib/view-transitions";

/**
 * Link che porta con sé la transizione a elemento condiviso lista → dettaglio
 * (DESIGN.md §7). Parametrico sull'entità: segnalazioni e opere condividono il
 * meccanismo, non lo duplicano — è la parte più sottile del codice di questa
 * app e due copie divergerebbero alla prima correzione.
 *
...
export function SharedElementLink({
  ...
  href: string;
  /** Nomi e attributi dell'entità: vengono da `CONDIVISO` in `lib/view-transitions`. */
  target: ElementoCondiviso;
  className?: string;
  children: ReactNode;
}) {
  ...
  function onClick(e: React.MouseEvent<HTMLAnchorElement>) {
    ...
    const card = ref.current?.closest<HTMLElement>(`[${target.card}]`);
    if (!card) return;

    e.preventDefault();
    card.style.viewTransitionName = NOME_CONDIVISO;
      ...
      () =>
        new Promise<void>((resolve) => {
          ...
          const osservatore = new MutationObserver(() => {
            if (document.querySelector(`[${target.attr}]`)) fine();
          });
