...
### 4. L'Ondata 8 — ▶️ APERTA il 2026-08-09, dal nucleo

Lorenzo ha scelto di aprire **dal nucleo** (analytics, alert, moderazione
assistita), non dalla pipeline degli atti — che resta pronta: la sua domanda di
rischio ha già risposta in `docs/fonti-pagella.md` §1.3 (il portale esporta
**24 colonne in CSV, `Url atto` compreso**).

**Fatta la prima voce: le analytics operative.** Due letture sul cruscotto,
forma **B** scelta sui mockup iniettati. Dettaglio in CHANGELOG 0.42.0.

⚠️ **Due cose che restano fuori, e perché**, non «rimandate»:

- **Il trend per quartiere**: metà delle celle è sotto il campione minimo (5 su
  10), quindi la lettura sarebbe per metà muta. **Condizione: entra quando i
  quartieri sotto soglia saranno meno di un terzo** — che con dati veri arriva
  da sé.
- **L'alert su trend anomalo**: le ultime due settimane del seed fanno 6 e 9
  contro una media di 2,8 e sembrano un picco da manuale, ma **i bucket del
  seed sono ancorati al calendario** (problema noto 25): tarare un'euristica lì
  significa tararla sulla semina. **Condizione: si scrive su una serie che non
...
I **Lavori 1, 2 e la parte possibile del 3 sono chiusi**, e l'**Ondata 8 è
aperta**: fatte le analytics operative (2026-08-09). Del Lavoro 3 resta solo
